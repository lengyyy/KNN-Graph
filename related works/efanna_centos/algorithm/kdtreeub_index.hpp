...
public:
	...
	void buildIndexImpl(){
		...
		s = clock();
#endif
		memoryInfo["memory_before_tree"]=getMemory();
		initGraph();
...
#ifdef INFO
		int  t_init = (f-s)*1.0/CLOCKS_PER_SEC;
		std::cout << "initial graph using time: "<< t_init <<" seconds"<< std::endl;
		myInfo["t_init"]=to_string(t_init);
#endif
		...
		memoryInfo["memory_after_graph"]=getMemory();
		ProfilerStop();
...
#pragma omp parallel for
		...
		memoryInfo["memory_after_tree"]=getMemory();
		memoryInfo["memory_before_graph"]=getMemory();
