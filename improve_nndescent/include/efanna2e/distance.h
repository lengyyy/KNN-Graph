...
#define SSE_L2NORM(addr, dest, tmp) \
    ...
    while (a < last) {
      ...
      float norm_partial(const float* a, unsigned size, unsigned div, float &p_right_size) const{
          //返回全部的平方和，还有赋值右半部分的平方和
          float result = 0;
#ifdef __GNUC__
#ifdef __AVX__
#define AVX_L2NORM(addr, dest, tmp) \
    ...
    dest = _mm256_add_ps(dest, tmp);
          ...
          _mm256_storeu_ps(unpack, sum);
          float result_left = unpack[0] + unpack[1] + unpack[2] + unpack[3] + unpack[4] + unpack[5] + unpack[6] + unpack[7];
          ...
          _mm256_storeu_ps(unpack, sum);
          result = unpack[0] + unpack[1] + unpack[2] + unpack[3] + unpack[4] + unpack[5] + unpack[6] + unpack[7];
          p_right_size = sqrt(result-result_left);
#else
          #ifdef __SSE2__
#define SSE_L2NORM(addr, dest, tmp) \
    tmp = _mm128_loadu_ps(addr); \
